function [features, gparams, feature_info] = init_features_dagnn(features, gparams, is_color_image, img_sample_sz, size_mode)
...
for k = 1:length(features)
    ...
    elseif isequal(features{k}.getFeature, @get_threshold)
        features{k}.fparams.nDim = 1;
        features{k}.is_cell = false;
        features{k}.is_cnn = false;
    elseif isequal(features{k}.getFeature, @get_cnn_layers) || isequal(features{k}.getFeature, @get_OFcnn_layers) || isequal(features{k}.getFeature, @get_cnn_layers_pytorch)
        ...
        if ~isfield(features{k}.fparams, 'nn_type')
            features{k}.fparams.nn_type = 'simplenn';
        end
        
        % load the network
        if strcmp(features{k}.fparams.nn_type,'simplenn')
            ...
            var_sizes = net.getVarSizes({'data',[net.meta.normalization.imageSize]});
            
            % Get output dims for each layer
            output_var_ids = 141;
            output_feat_sizes = reshape(cell2mat(var_sizes(output_var_ids)),4,[]);
            features{k}.fparams.nDim = output_feat_sizes(3,:)';
            
            % Find the strides
            rfs_all = net.getVarReceptiveFields('data');
            output_strides = reshape([rfs_all(output_var_ids).stride],2,[]);
            
            % Find the cell size for the output layers
            features{k}.fparams.cell_size = output_strides(1,:)'.* features{k}.fparams.downsample_factor';
        end
...
if cnn_feature_ind > 0
    ...
    is_simplenn = strcmp(features{cnn_feature_ind}.fparams.nn_type, 'simplenn');
    ...
    if ~strcmpi(size_mode, 'same') && strcmpi(features{cnn_feature_ind}.fparams.input_size_mode, 'adaptive')
        if is_simplenn
            ...
            current_last_layer_sz = net.info.dataSize(1:2,end);
        else
            % Find the size of the output layer with lowest resolution
            output_var_ids = 141;
            
            var_sizes = net.getVarSizes({'data',[net.meta.normalization.imageSize]});
            output_feat_sizes = reshape([var_sizes{output_var_ids}],4,[]);
            
            [~, lowest_res_id] = min(output_feat_sizes(1,:));
            
            lowest_ouput_var_id = output_var_ids(lowest_res_id);
            
            current_last_layer_sz = output_feat_sizes(1:2,lowest_res_id);
            
            orig_sz = current_last_layer_sz / features{cnn_feature_ind}.fparams.downsample_factor(lowest_res_id);
        end
        ...
        while desired_sz(1) > current_last_layer_sz(1)
            new_img_sample_sz = new_img_sample_sz + [1, 0];
            
            if is_simplenn
                net_info = vl_simplenn_display(net, 'inputSize', [round(scale * new_img_sample_sz), 3 1]);
                current_last_layer_sz = net_info.dataSize(1:2,end);
            else
                var_sizes = net.getVarSizes({'data',[round(scale * new_img_sample_sz), 3 1]});
                
                % Find the size of the output layer with lowest resolution
                current_last_layer_sz = var_sizes{lowest_ouput_var_id}(1:2);
            end
        end
        while desired_sz(2) >current_last_layer_sz(2)
    ...
    if is_simplenn
        ...
        net_stride = output_strides';
        total_feat_sz = output_feat_sizes(1:2,:)';
    end

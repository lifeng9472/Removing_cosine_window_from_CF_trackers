function [merged_sample, new_sample, merged_sample_id, new_sample_id, distance_matrix, gram_matrix, prior_weights] = ...
    ...
    num_training_samples,learning_rate, params, sample_merge_type, minimum_sample_weight)
...
if num_training_samples == params.nSamples
    ...
    if min_sample_weight < minimum_sample_weight
        ...
        prior_weights = prior_weights*(1 - learning_rate)/sum(prior_weights);
        ...
        if new_sample_min_dist < existing_samples_min_dist
            ...
            merged_sample = merge_samples(existing_sample_to_merge, new_train_sample, prior_weights(merged_sample_id), learning_rate, sample_merge_type);
            
            % Update distance matrix and the gram matrix
            [distance_matrix, gram_matrix] = update_distance_matrix(distance_matrix, gram_matrix, gram_vector, new_train_sample_norm, merged_sample_id, -1, prior_weights(merged_sample_id), learning_rate);
            
            % Update the prior weight of the merged sample
            prior_weights(closest_sample_to_new_sample) = prior_weights(closest_sample_to_new_sample) + learning_rate;          
        else
            ...
            merged_sample = merge_samples(sample_to_merge1, sample_to_merge2, prior_weights(closest_existing_sample1), prior_weights(closest_existing_sample2), sample_merge_type);
            ...
            prior_weights(closest_existing_sample2) = learning_rate;

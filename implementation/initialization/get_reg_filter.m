function reg_filter = get_reg_filter(sz, target_sz, params, reg_params)

% Compute the spatial regularization function and derive the corresponding
% filter operation used for the optimization
reg_window_edge = reg_params.reg_window_edge;
reg_window_min = reg_params.reg_window_min;
reg_sparsity_threshold = reg_params.reg_sparsity_threshold;
...
if params.use_reg_window
    ...
    reg_window_dft(abs(reg_window_dft) < reg_sparsity_threshold * max(abs(reg_window_dft(:)))) = 0;
    
    % do the inverse transform, correct window minimum
    reg_window_sparse = real(ifft2(reg_window_dft));
    reg_window_dft(1,1) = reg_window_dft(1,1) - prod(sz) * min(reg_window_sparse(:)) + reg_window_min;
    ...
    reg_filter = cast(reg_window_min, 'like', params.data_type);
end
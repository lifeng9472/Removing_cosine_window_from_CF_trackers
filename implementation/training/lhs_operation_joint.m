function hf_out = lhs_operation_joint(hf, samplesf_proj, sample_weights,reg_filter, samplesf, init_hf, proj_reg, tau)
...
sh = mtimesx(samplesf_proj{k1}, permute(hf{k1}, [3 4 1 2]), 'speed');    % assumes the feature with the highest resolution is first
for k = block_inds
    pad_sz{k} = (output_sz - [filter_sz(k,1), 2*filter_sz(k,2)-1]) / 2;
    
    sh(:,1,1+pad_sz{k}(1):end-pad_sz{k}(1), 1+pad_sz{k}(2):end) = ...
        sh(:,1,1+pad_sz{k}(1):end-pad_sz{k}(1), 1+pad_sz{k}(2):end) + mtimesx(samplesf_proj{k}, permute(hf{k}, [3 4 1 2]), 'speed');
end
...
hf_out1{k1} = tau * permute(conj(mtimesx(sh, 'C', samplesf_proj{k1}, 'speed')), [3 4 2 1]);
for k = block_inds
    hf_out1{k} = tau * permute(conj(mtimesx(sh(:,1,1+pad_sz{k}(1):end-pad_sz{k}(1), 1+pad_sz{k}(2):end), 'C', samplesf_proj{k}, 'speed')), [3 4 2 1]);
end
...
for k = 1:num_features
    BP_cell{k} = mtimesx(mtimesx(samplesf{k}, P{k}, 'speed'), init_hf{k}, 'speed');
end
...
% Multiply by weight
BP = bsxfun(@times,sample_weights,BP);

% multiply with the transpose: A^H * BP
% hf_out{1,1,k1} = hf_out1{k1} +  permute(bsxfun(@times, BP, conj(samplesf_proj{k1})), [3 4 2 1]);
hf_out{1,1,k1} = hf_out1{1,1,k1} + tau * permute(conj(mtimesx(BP, 'C', samplesf_proj{k1}, 'speed')), [3 4 2 1]);

% B^H * BP
fBP = cell(1,1,num_features);
% fBP{k1} = reshape(bsxfun(@times, conj(init_hf{k1}), BP), size(init_hf{k1},1), []).';

fBP{k1} = reshape(mtimesx( samplesf{k1}, 'C',BP, 'speed'), size(samplesf{k1},2), []);
...
shBP{k1} = reshape(mtimesx( samplesf{k1}, 'C',sh, 'speed'), size(samplesf{k1},2), []);

for k = block_inds
    % multiply with the transpose: A^H * BP
    hf_out{1,1,k} = hf_out1{k} + tau * permute(conj(mtimesx(BP(:,1,1+pad_sz{k}(1):end-pad_sz{k}(1), 1+pad_sz{k}(2):end), 'C',samplesf_proj{k}, 'speed')), [3 4 2 1]);
    
    % B^H * BP
    % fBP{k} = reshape(bsxfun(@times, conj(init_hf{k}), BP(1,1,1+pad_sz{k}(1):end-pad_sz{k}(1), 1+pad_sz{k}(2):end)), size(init_hf{k},1), []).';
    fBP{k} = reshape(mtimesx( samplesf{k}, 'C',BP(:,1,1+pad_sz{k}(1):end-pad_sz{k}(1), 1+pad_sz{k}(2):end), 'speed'),size(samplesf{k},2), []);
    
    % Compute proj matrix part: B^H * A_m * f
    % shBP{k} = reshape(bsxfun(@times, conj(init_hf{k}), sh(1,1,1+pad_sz{k}(1):end-pad_sz{k}(1), 1+pad_sz{k}(2):end)), size(init_hf{k},1), []).';
    shBP{k} = reshape(mtimesx( samplesf{k}, 'C',sh(:,1,1+pad_sz{k}(1):end-pad_sz{k}(1), 1+pad_sz{k}(2):end), 'speed'),size(samplesf{k},2), []);
end
...
for k = 1:num_features
    ...
    % hf_out2 = 2*real(XH{k} * fBP{k} - XH{k}(:,fi:end) * fBP{k}(fi:end,:)) + proj_reg * P{k};
    init_hf_h = reshape(init_hf{k},size(init_hf{k},1), [])';
    hf_out2 = tau * 2*real(fBP{k}*init_hf_h - fBP{k}(:,fi:end)*init_hf_h(fi:end,:)) + proj_reg * P{k};
    
    % Compute proj matrix part: B^H * A_m * f
    % hf_out{2,1,k} = hf_out2 + (2*real(XH{k} * shBP{k} - XH{k}(:,fi:end) * shBP{k}(fi:end,:)));
    hf_out{2,1,k} = hf_out2 + tau * (2*real(shBP{k} * init_hf_h - shBP{k}(:,fi:end) * init_hf_h(fi:end,:)));
end
end
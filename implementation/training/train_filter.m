function [hf, res_norms, CG_state] = train_filter(hf, samplesf, yf, reg_filter, sample_weights, sample_energy, reg_energy, params, CG_opts, CG_state, tau)
...
rhs_samplef = cellfun(@(xf, yf) bsxfun(@times, conj(permute(xf,[3 4 1 2])), yf), samplesf, yf, 'uniformoutput', false);
rhs_samplef = cellfun(@(x) bsxfun(@times, x, permute(sample_weights,[3 4 1 2])), rhs_samplef, 'uniformoutput', false);
rhs_samplef = cellfun(@(x) tau * squeeze(sum(x,3)),rhs_samplef, 'uniformoutput', false);
...
[hf, res_norms, CG_state] = pcg_ccot(...
    @(x) lhs_operation(x, samplesf, reg_filter, sample_weights, tau),...
    rhs_samplef, CG_opts, ...
